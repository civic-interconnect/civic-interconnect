/// Build script for cep-core crate.
/// Generates Rust code to include JSON and YAML assets
/// from the repository into the compiled crate.
///
/// The generated code is written to `OUT_DIR/assets_generated.rs`
/// and included in the crate source.
///
/// The assets are organized into static arrays:
/// - SCHEMAS: JSON Schema files from `schemas/`
/// - VOCABULARY: Vocabulary JSON files from `vocabulary/`
/// - LOCALIZATION_YAMLS: YAML localization files from `localization/`
/// - TEST_VECTORS: Test vector JSON files from `test_vectors/`
/// /// Each array contains tuples of (key, content),
/// where `key` is derived from the file path according to specific rules.
/// use std::collections::BTreeMap;
///
/// Path: src/rust/cep-core/build.rs
use std::env;
use std::fs;
use std::io::Write;
...
fn is_repo_root(dir: &Path) -> bool {
    ...
    if !dir.join("mkdocs.yml").is_file() {
        return false;
    }
...
fn main() {
    ...
    // Find the Cargo workspace root (Cargo.toml containing [workspace]).
    let repo_root = find_repo_root(&manifest_dir).unwrap_or_else(|| {
        panic!(
            "Could not locate Cargo workspace root starting from {}",
            manifest_dir.display()
        )
    });

    // // Surface what root we found in build logs.
    // println!(
    //     "cargo:warning=cep-core build.rs using repo_root={}",
    //     repo_root.display()
    // );
    ...
    // Fail loudly if required asset folders are missing.
    for folder in ["schemas", "vocabulary", "localization", "test_vectors"] {
        let p = repo_root.join(folder);
        if !p.is_dir() {
            panic!(
                "build.rs: required folder '{}' not found at {} (repo_root={})",
                folder,
                p.display(),
                repo_root.display()
            );
        ...
        println!(
            "cargo:warning=cep-core build.rs found {} at {}",
            folder,
            p.display()
        );
    ...
    // vocabulary -> VOCABULARY (relative path without .json)
